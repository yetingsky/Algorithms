...
//        energy = new double[picture.height()][picture.width()];
        ...
        transposed = false;
//        if (height > width)
//            energyCache = new double[height];
//        else 
//            energyCache = new double[width];
...
//                newpic.set(i, j, color[j][i]);
    ...
    public int[] findVerticalSeam() {
        ...
        double[] cache = new double[widthR()];
...
//        System.out.println(edgeTo[height()-1][minIndex]);
    ...
    private void relax(int i, int j, double[][] disTo, int[][] edgeTo, double[] cache) {
//        double energy1 = getEnergy(i+1, j-1);
//        double energy2 = getEnergy(i+1, j);
//        double energy3 = getEnergy(i+1, j+1);
        if (disTo[i+1][j-1] > disTo[i][j] + cache[j-1]) {
            disTo[i+1][j-1] = disTo[i][j] + cache[j-1];
            edgeTo[i+1][j-1] = j;
        }
            
        if (disTo[i+1][j] > disTo[i][j] + cache[j]) {
            disTo[i+1][j] = disTo[i][j] + cache[j];
            edgeTo[i+1][j] = j; 
        }
    ...
    private void setCache(int i, double[] cache) {
        for (int j = 0; j < widthR(); j++) {
            cache[j] = getEnergy(i+1, j);
        }
    }
